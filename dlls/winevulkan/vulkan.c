        ...
        struct conversion_context *ctx, const VkDeviceCreateInfo *src, VkDeviceCreateInfo *dst)
{
    ...
    *dst = *src;
    ...
    if (phys_dev->external_memory_align || phys_dev->map_placed_align)
    {
        const char **new_extensions;
        new_extensions = conversion_context_alloc(ctx, (dst->enabledExtensionCount + 2) * sizeof(*dst->ppEnabledExtensionNames));
